...
#include <common.h>
#include <asm/arch/dma_i_sunxi.h>
#include <asm/arch/dma_sunxi.h>

//@mhoffrog - added union definitions to avoid pointer alignment warnings below
typedef union {
	uint uiVal;
	__ddma_config_t cfg;
} __ddma_config_u;
...
int DMA_Setting(uint hDMA, void *cfg)
{
    ...
    if(pDma->other)
    {
    	//@mhoffrog - use union type
        __ddma_config_u    ddma_arg = {0};

        ddma_arg.cfg.src_drq_type     = arg->cfg.src_drq_type;
        ddma_arg.cfg.src_addr_type    = arg->cfg.src_addr_type;
        ddma_arg.cfg.src_burst_length = arg->cfg.src_burst_length;
        ddma_arg.cfg.src_data_width   = arg->cfg.src_data_width;

        ddma_arg.cfg.dst_drq_type     = arg->cfg.dst_drq_type;
        ddma_arg.cfg.dst_addr_type    = arg->cfg.dst_addr_type;
        ddma_arg.cfg.dst_burst_length = arg->cfg.dst_burst_length;
        ddma_arg.cfg.dst_data_width   = arg->cfg.dst_data_width;

        ddma_arg.cfg.continuous_mode  = arg->cfg.continuous_mode;

    	//@mhoffrog - use uiVal of union
        pDma->channal->config       = ddma_arg.uiVal & (~0x80000000);
    ...
    {
    	//@mhoffrog - use union type
        __ndma_config_u    ndma_arg = {0};

        ndma_arg.cfg.src_drq_type     = arg->cfg.src_drq_type;
        ndma_arg.cfg.src_addr_type    = arg->cfg.src_addr_type;
        ndma_arg.cfg.src_burst_length = arg->cfg.src_burst_length;
        ndma_arg.cfg.src_data_width   = arg->cfg.src_data_width;

        ndma_arg.cfg.dst_drq_type     = arg->cfg.dst_drq_type;
        ndma_arg.cfg.dst_addr_type    = arg->cfg.dst_addr_type;
        ndma_arg.cfg.dst_burst_length = arg->cfg.dst_burst_length;
        ndma_arg.cfg.dst_data_width   = arg->cfg.dst_data_width;

        ndma_arg.cfg.continuous_mode  = arg->cfg.continuous_mode;
        ndma_arg.cfg.wait_state       = arg->cfg.wait_state;

    	//@mhoffrog - use uiVal of union
        pDma->channal->config     = ndma_arg.uiVal & (~0x80000000);
    }
